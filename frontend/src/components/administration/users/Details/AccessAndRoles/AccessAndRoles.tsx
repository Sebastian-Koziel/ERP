...
export async function updateAccess({ request }: { request: Request }) {
  ...
  if (data.get("accessUsersTab") === "on") {
    usersNavGeneralAccess = true;
  } else {
    usersNavGeneralAccess = false;
  }
  let stagesGeneralAccess;
  if (data.get("accessStagesTab") === "on") {
    stagesGeneralAccess = true;
  } else {
    stagesGeneralAccess = false;
  }
